...
export class ThreadPool extends RemoteAwareMixin(ListenerAwareMixin()) {
     ...
     */
    sendMessageToRemote(threadId, data = {}, transferrable = undefined) {
        if (typeof data === 'object') {
            data.thread = this.id;
        }
        if (this.isMasterThread(threadId)) {
            return this.sendMessageToMaster(data, transferrable);
        }
        if (this.isThisThread(threadId) || !this.hasRemotes()) {
            this.messageListener(this.createMessageEvent(data));
            return true;
        }
        return super.sendMessageToRemote(threadId, data, transferrable);
    }
    ...
    executeCommand(evt) {
        ...
        response
            ...
            .then(data => this.sendMessageToRemote(thread, { data, returnId: callbackId }))
